...
def preprocess_data(pointcloud_frustum_data, bb_label_data, centroids=[], width_meters=15, height_meters=20, resolution=0.1, bb_shift=75):
    ...
    for counter, (data_point, centroid) in enumerate(zip(data, centroids)):
        ...
        target_data[counter] = [labels[counter][0] + bb_shift - centroid_x,
                                labels[counter][1],
                                labels[counter][2] + bb_shift - centroid_z,
                                labels[counter][3],
                                labels[counter][4],
                                labels[counter][5],
                                labels[counter][6]]
...
def test():
    ...
    model = keras.models.load_model('saved_model_hist_norm_1')

    test_target = validation_labels[faraway_indices[3]] # ground truth untouched
    test_centroid = validation_centroids[faraway_indices[3]] # need to change input shape of val data to use the trained NN
    test_data = validation_input_data[faraway_indices[3]]
    ...
    print('Test label X:', test_target[0],
          'Test label Y:', test_target[1],
          'Test label Z:', test_target[2],
          'Test label REST:', test_target[3:],  '\n',

          'Pred. label X:', predicted_labels[0, 0] - bb_shift + test_centroid[0],
          'Pred. label Y:', predicted_labels[0, 1],
          'Pred. label Z:', predicted_labels[0, 2] - bb_shift + test_centroid[2],
          'Pred. label REST:', predicted_labels[0, 3:])
