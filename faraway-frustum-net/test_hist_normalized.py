...
def test():
    ...
    validation_data = pickle.load(
        open('/localdata/yurtsever.2/Data/faraway_frustum/data_split01_val_Car/samples.p', 'rb'))
    validation_labels = pickle.load(
        open('/localdata/yurtsever.2/Data/faraway_frustum/data_split01_val_Car/labels.p', 'rb'))
    ...
    model = keras.models.load_model('saved_models/car_mobile_net')

    test_target = validation_labels[faraway_indices[10]] # ground truth untouched
    test_centroid = validation_centroids[faraway_indices[10]] # need to change input shape of val data to use the trained NN
    test_data = validation_input_data[faraway_indices[10]]
    ...
    print('Test label X:', test_target[0] , 'Test label Y:', test_target[1], 'Test label Z:', test_target[2],
          ...
          'Pred. label Y:', predicted_labels[0, 1], 'Pred. label Z:', predicted_labels[0, 2] - bb_shift + test_centroid[2],
          'Pred. label REST:', predicted_labels[0,3:6], predicted_labels[0,6] - 3.14)
    print(test_centroid)
