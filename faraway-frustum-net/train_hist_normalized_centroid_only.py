...
bb_shift=75
data = pickle.load(open('/localdata/yurtsever.2/Data/faraway_frustum/data_split01_train_Pedestrian/samples.p', 'rb'))
labels = pickle.load(open('/localdata/yurtsever.2/Data/faraway_frustum/data_split01_train_Pedestrian/labels.p', 'rb'))

assert len(data) == len(labels)
indices = get_nonZero_indices(data)

data = [data[i] for i in indices]
labels = [labels[i] for i in indices]

validation_data = pickle.load(open('/localdata/yurtsever.2/Data/faraway_frustum/data_split01_val_Pedestrian/samples.p', 'rb'))
validation_labels = pickle.load(open('/localdata/yurtsever.2/Data/faraway_frustum/data_split01_val_Pedestrian/labels.p', 'rb'))
...
training_input_data, training_target_data = preprocess_data(data, labels, centroids, bb_shift=bb_shift)
validation_input_data, validation_target_data = preprocess_data(validation_data, validation_labels, validation_centroids, bb_shift=bb_shift)
...
#model = BEV2dCentroid([training_input_data.shape[1], training_input_data.shape[2], 1]).model
model_maker = BEV2dCentroid([training_input_data.shape[1], training_input_data.shape[2], 1])
model = model_maker._build_model_centroid()

loss_fn = keras.losses.MeanAbsoluteError()
metric = keras.metrics.MeanAbsoluteError()

callback = tf.keras.callbacks.EarlyStopping(monitor="val_loss", restore_best_weights=True, patience=10, baseline=1.5)
...
model.save('saved_models/saved_model_hist_norm_centroid_1')

test_target = validation_target_data[0]
test_centroid = validation_centroids[0]
