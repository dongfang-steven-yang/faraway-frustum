...
1. Open `data_process.py` to process raw detection result files and corresponding KITTI label files:    
   $\star$ Note: if you want to use the detection results given in `result` folder, you do not need to do following $\ast$ steps since the given results of our method in `result` are already fused with an state-of-the-art detector(PV-RCNN). If you want to use the results generated by `step2_get_kitti_results.py` or your own model, please do $\ast$ steps.   
   $\ast$ Give the PATH to our raw detection result files (e.g 000000.txt ...) and the PATH to state-of-the-art detector's results (e.g 000000.txt ...) in `line 417-418`. 
   $\ast$ Give `fuction='fuse_result'` in `line 404`  and then run the code to generate our detection result files by fusing our faraway object results with state-of-the-art detector's results. 
