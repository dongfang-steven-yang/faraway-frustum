...
from utils.rotate_iou import rotate_iou_gpu_eval
...
def get_mAP_v2(prec):
    ...
    #     sums = sums + prec[..., i]
    # return sums / 11 * 100
    sums=[sum(prec[0]) / 41 * 100,sum(prec[1]) / 41 * 100 ,sum(prec[2]) / 41 * 100]
    return sums
...
def get_official_eval_result(gt_annos,
    ...
    overlap_mod = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7, 0.7],
                            ...
                            [0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7, 0.7]])
    overlap_easy = np.array([[0.5, 0.25, 0.25, 0.7, 0.5, 0.5, 0.5, 0.5],
                            [0.5, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5, 0.5],
                            [0.5, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5, 0.5]])
    overlap_veryeasy =np.array([[0.25, 0.01, 0.01, 0.7, 0.5, 0.5, 0.5, 0.5],
                            [0.25, 0.01, 0.01, 0.5, 0.25, 0.5, 0.5, 0.5],
                            [0.25, 0.01, 0.01, 0.5, 0.25, 0.5, 0.5, 0.5]])
    min_overlaps = np.stack([overlap_mod, overlap_easy,overlap_veryeasy], axis=0)  # [2, 3, 5]
    class_to_name = {
        0: 'Car',
        1: 'Pedestrian',
        2: 'Cyclist',
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            result += print_str(
                (f"{class_to_name[curcls]} "
                 "AP(Average Precision) under acceptable IOU for easy, mod, hard: @{:.2f}, {:.2f}, {:.2f}:".format(*min_overlaps[i, :, j])))
